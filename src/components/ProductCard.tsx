...
const ProductCard: React.FC<{ product: Product }> = ({ product }) => {
    const { FaStar } = icons
    let slug = `${(product.title!).replace(/\s+/g, '-')}-${product._id}`
                ...
                <div className='relative'>
                    {product!.sale! > 0 &&
                        <div className='absolute top-0 right-0 z-50'>
                            <div className='text-white bg-red-500 flex items-center border rounded-tr-md rounded-bl-md px-3 py-1.5'>
                                <span className='text-sm text-center font-semibold'>-{product.sale}%</span>
                            </div>
                        </div>
                    }
                    <Link className='text-decoration-none' href={`/product/${slug}`}>
                        <div className='aspect-[1/1.1] w-full relative overflow-hidden rounded-t-md'>
                            <Image fill loading='lazy' className='object-cover group-hover:scale-105 transition-transform duration-1000' src={product.images![0]} alt={product.title!} sizes='width: 100%, height: 100%' />
                        </div>
                        <div className='py-1 px-2'>
                            <span className='text-wrap line-clamp-2 text-bold text-sm h-[calc(20px*2)]'>{product.title}</span>
                            <div className='mt-2'>
